...
import net.arccode.wechat.pay.api.common.mapping.ACFieldMethod;
...
public class Converters {
    ...
    static {
        baseFields.add("returnCode");
        baseFields.add("returnMsg");
    }
     ...
     */
    public static <T> T convert(Class<T> clazz, Reader reader) throws WXPayApiException {
        T rsp = null;
        ...
        try {
            ...
            boolean isResponseClazz = WXPayResponse.class.isAssignableFrom(clazz);
            ...
            for (PropertyDescriptor pd : pds) {
                ...
                List<ACFieldMethod> ACFieldMethods = new ArrayList<ACFieldMethod>();

                if (baseFields.contains(itemName) && isResponseClazz) {

                    Field field = WXPayResponse.class.getDeclaredField(itemName);

                    ACFieldMethod ACFieldMethod = new ACFieldMethod();
                    ACFieldMethod.setField(field);

                    // writeMethod属于父类，则直接使用
                    if (writeMethod.getDeclaringClass().getName().contains("WXPayResponse")) {

                        ACFieldMethod.setMethod(writeMethod);
                    } else {
                        // 否则从父类再取一次
                        writeMethod = tryGetSetMethod(WXPayResponse.class, field,
                                writeMethod.getName());
                        if (writeMethod == null) {
                            continue;
                        }
                    ...
                    if (overideFields.contains(itemName)) {
                        ...
                        if (field != null) {
                            ...
                            ACFieldMethod = new ACFieldMethod();
                            ACFieldMethod.setField(field);
                            ACFieldMethod.setMethod(writeMethod);
                            ACFieldMethods.add(ACFieldMethod);
                        }
                ...
                // 迭代设置属性
                for (ACFieldMethod ACFieldMethod : ACFieldMethods) {

                    Field field = ACFieldMethod.getField();
                    Method method = ACFieldMethod.getMethod();
                    ...
                    if (String.class.isAssignableFrom(typeClass)) {
                        ...
                        if (value instanceof String) {
                            ...
                            if (isCheckJsonType && value != null) {
                                throw new WXPayApiException(itemName + " is not a String");
                            }
                            if (value != null) {
                                method.invoke(rsp, value.toString());
                            } else {
                        ...
                        if (value instanceof Long) {
                            ...
                            if (isCheckJsonType && value != null) {
                                throw new WXPayApiException(itemName + " is not a Number(Long)");
                            }
                        ...
                        if (value instanceof Integer) {
                            ...
                            if (isCheckJsonType && value != null) {
                                throw new WXPayApiException(itemName + " is not a Number(Integer)");
                            }
                            if (StringUtils.isNumeric(value)) {
                        ...
                        if (value instanceof Boolean) {
                            ...
                            if (isCheckJsonType && value != null) {
                                throw new WXPayApiException(itemName + " is not a Boolean");
                            }
                            if (value != null) {
                        ...
                        if (value instanceof Double) {
                            ...
                            if (isCheckJsonType && value != null) {
                                throw new WXPayApiException(itemName + " is not a Double");
                            }
                        }
                        ...
                        if (value instanceof Number) {
                            ...
                            if (isCheckJsonType && value != null) {
                                throw new WXPayApiException(itemName + " is not a Number");
                            }
                        }
